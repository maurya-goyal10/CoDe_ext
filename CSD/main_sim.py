...
# Changes starts >>>>>>>>>>>>>>>>>>>>>>>>>
from data.custom import CustomD 
# Changes ends <<<<<<<<<<<<<<<<<<<<<<<<<<<
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.dataset == 'custom':
        dataset_query = CustomD(args.data_dir, ret_transform)
        dataset_values = CustomD(args.data_dir, ret_transform)
    # Changes ends <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
