...
def main():
    ...
    parser.add_argument('--style_folder', default='./data/stroke_gen/')
    ...
    prompts = []
    if opt.text != None:
        prompts.append(opt.text)
    else:
        if opt.text_type == 0:
            prompts.append("the skyline of New York City")
        elif opt.text_type == 1:
            prompts.append("a Big Ben clock towering over the city of London")
        elif opt.text_type == 2:
            prompts.append("a Scottish castle next to a loch")
        elif opt.text_type == 3:
            prompts.append("a photo of a medievel church")
    ...
    if opt.text == None and opt.text_type == None:
        with open(ASSETS_PATH.joinpath('stroke.txt'), 'r') as fp:
