...
def main():
    ...
    name_file = 'ablations/just_checking_fid_new'
    ...
    for source_dir in tqdm(source_dirs):
        ...
        for target_dir in target_dirs:
            ...
            tmp_root = outputs_path / "tmp"
            all_prompt_path = tmp_root / source_dir.stem / "all_prompts"
            all_uncond_path = tmp_root / "uncond2" / "all_uncond"
            
            all_prompt_path.mkdir(parents=True, exist_ok=True)
            all_uncond_path.mkdir(parents=True, exist_ok=True)            
            ...
            for prompt_dir in prompt_dirs:
                ...
                try:
                    ...
                    # Copy prompt images with unique names
                    for img_path in prompt_dir.glob('*.png'):
                        new_name = f'{prompt_dir.stem}_{img_path.name}'
                        shutil.copy(img_path, all_prompt_path / new_name)

                    # Copy uncond images with unique names
                    for img_path in uncond_path_p.glob('*.png'):
                        new_name = f'{prompt_dir.stem}_{img_path.name}'
                        shutil.copy(img_path, all_uncond_path / new_name)

                    # out = os.popen(f"python {root_path}/pytorch-fid/src/pytorch_fid/fid_score.py '{all_uncond_path.as_posix()}' '{all_prompt_path.as_posix()}'").read()
                    ...
                    print(f'Base divergence {prompt_dir.as_posix()}: {e}')
                    continue
