...
class CoDeSDExtensionPipeline(StableDiffusionPipeline):
    @torch.no_grad()
    def __call__(
        self,
        offset: int = 5,
        ...
        for batch_iter in tqdm(range(num_batch), total=num_batch):
            ...
            for i, t in enumerate(timesteps):
                ...
                # don't do this for the next step after greedy sampling as have calculated the noise when predicted \
                # the x_0 and can use that
                if(i%block_size != 0 or i==0):
                ...
                # perform guidance
                    ...
                    print(f"Not calculating at {i} or at {t.item()}")
                ...
                if ((i + 1) % block_size == 0) or (t == timesteps[-1]): # at the end of block do BoN
                    print(f"Sampling at {i} or at {t.item()}")
