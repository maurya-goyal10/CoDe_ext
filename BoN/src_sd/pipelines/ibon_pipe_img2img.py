...
#
# ---------------------------------------------------------------------------
# Contains the implementation of the pipeline for Intermediate-BoN
# ---------------------------------------------------------------------------
...
class IBoNSDPipelineI2I(StableDiffusionPipeline):
    @torch.no_grad()
    def __call__(
        self,
        offset: int = 5,
        ...
        for batch_iter in tqdm(range(num_batch), total=num_batch):
            ...
            for i, t in enumerate(timesteps):
                ...
                if (t > timesteps[-1]) and (i % block_size == 0):
                    ...
                    for iter in range(n_samples):
                        ...
                        latent_temp = copy.deepcopy(curr_samples)
                        ...
                        pred_original_temp = self.scheduler.step(noise_pred_temp, timesteps[i + 1], latent_temp, **extra_step_kwargs).pred_original_sample

                        try:
                            gen_rewards.append(self.compute_scores(pred_original_temp, prompt).unsqueeze(0))
                        except:
                            gen_failed = True
                            break

                        gen_samples.append(copy.deepcopy(latent_temp.unsqueeze(0)))
                    ...
                    # print(gen_samples[0][select_ind[0]].shape)
                    curr_samples = copy.deepcopy(torch.cat([x[select_ind[idx]].unsqueeze(0) for idx, x in enumerate(gen_samples)], dim=0))
