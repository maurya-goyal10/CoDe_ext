...
class CoDeGradNewSD(StableDiffusionPipeline):
    @torch.no_grad()
    def __call__(
        self,
        offset: int = 5,
        ...
        for batch_iter in tqdm(range(num_batch), total=num_batch):
            ...
            for i, t in enumerate(timesteps):
                ...
                if ((i-1)%block_size == 0) and (t<self.start_time*1000) and (t>= self.end_time*1000) and (self.target_guidance > 0.0):
                    grad = self.compute_gradient(curr_samples[0], prompt, noise_pred[0], t)
                    print(grad.shape)
                    ...
                    curr_samples[0] = curr_samples[0] + target_guidance*grad
