...
class CoDeGradSDExtension(StableDiffusionPipeline):
    @torch.no_grad()
    def __call__(
        self,
        offset: int = 5,
        ...
        for batch_iter in tqdm(range(num_batch), total=num_batch):
            ...
            for i, t in enumerate(timesteps):
                ...
                if ((i+1)%self.blocksize_gradient == 0 and (i+1)%block_size != 0) and \
                t<self.start_time*1000 and t>= self.end_time*1000 and self.target_guidance > 0.0:
                    print(f"{t.item()} is gradient guided")
                    for i in range(n_samples):
                            grad = self.compute_gradient(curr_samples[i], prompt, noise_pred, t)
                            ...
                            if target_guidance > 150.0: target_guidance = 150
                            # print(f"t: {t.item()}, the target_guidance is {target_guidance} the grad norm is {grad_norm}")
                            curr_samples[i] = curr_samples[i] + target_guidance*grad
                ...
                if ((i + 1) % block_size == 0) or (t == timesteps[-1]): # at the end of block do BoN
                    ...
                    if t > timesteps[-1]: # If not the end replicate n times
                        if t<self.start_time*1000 and t>= self.end_time*1000 and self.target_guidance > 0.0 and (i+1)%self.blocksize_gradient == 0:
                            ...
                            print(f"{t.item()} doing gradient on greedy sample")
                        else:
                            print(f"{t.item()} just doing greedy sampling")
                        curr_samples = curr_samples.repeat(n_samples, 1, 1, 1) # (n_samples, 4, 64, 64)
    ...
    def set_grad_block_size(self,grad_block_size):
        self.blocksize_gradient = grad_block_size
