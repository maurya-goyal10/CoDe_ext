...
class CoDeGradNewSDVariant(StableDiffusionPipeline):
    @torch.no_grad()
    def __call__(
        self,
        offset: int = 5,
        ...
        for batch_iter in tqdm(range(num_batch), total=num_batch):
            ...
            for i, t in enumerate(timesteps):
                ...
                if ((i-1)%block_size == 0) and (t<self.start_time*1000) and (t>= self.end_time*1000) and (self.target_guidance > 0.0):
                    ...
                    # print(f"t:{t.item()} the selected index is {select_ind}")
                    grad = self.compute_gradient(curr_samples[select_ind], prompt, noise_pred[0], t)
                    print(grad.shape)
                    target_guidance = (correction * correction).mean().sqrt().item() * guidance_scale / ((grad * grad).mean().sqrt().item() + 1e-8) * self.target_guidance 
                    if target_guidance > 150: target_guidance = 150
                    curr_samples[select_ind] = curr_samples[select_ind] + target_guidance*grad
                ...
                if ((i + 1) % block_size == 0) or (t == timesteps[-1]) : # at the end of block do BoN
                    ...
                    if t > timesteps[-1]: # If not the end replicate n times
                        ...
                        #     target_guidance = (correction * correction).mean().sqrt().item() * guidance_scale / ((grad * grad).mean().sqrt().item() + 1e-8) * self.target_guidance 
                        #     print(f"t: {t.item()}, the target_guidance is {target_guidance}")
