...
#
# ---------------------------------------------------------------------------
# Contains the implementation of the pipeline for BoN
# ---------------------------------------------------------------------------
...
class BoNSDPipelineI2I(StableDiffusionPipeline):
    @torch.no_grad()
    def __call__(
        self,
        offset: int = 5,
        ...
        for batch_iter in tqdm(range(num_batch), total=num_batch):
            ...
            curr_samples = torch.cat([x.unsqueeze(0) for x in curr_samples.chunk(n_samples)], dim=0) # (n_samples, self.genbatch, 4, 64, 64)
            curr_samples = curr_samples.permute(1,0,2,3,4)
            result = torch.cat([x[select_ind[idx]].unsqueeze(0) for idx, x in enumerate(curr_samples)], dim=0) # TODO: Make it efficient

            # try:
            self.save_outputs(result, start=(batch_iter*self.genbatch)+offset, prompt=prompt, num_try=num_try)
            # except:
            #     is_failed = True
            #     continue
