...
def run_experiment(config):
    ...
    if config.guidance.method == "bon":
        ...
        pipe = IBoNSDPipeline.from_pretrained(
            model_id, torch_dtype=torch.float16).to(device)
    elif config.guidance.method == "code":
    ...
    if isinstance(scorer, ClipScorer) and config.guidance.scorer != 'strokegen':
        ...
        target_map = {
            "0": '../assets/stroke_gen/lsun_church/stroke_img_0.jpg',
            "1": '../assets/stroke_gen/lsun_church/stroke_img_1.jpg',
            "2": '../assets/stroke_gen/lsun_church/stroke_img_2.jpg',
            "3": '../assets/stroke_gen/lsun_church/stroke_img_3.jpg',
            "4": '../assets/stroke_gen/lsun_church/stroke_img_4.jpg',
        }
    ...
    if isinstance(pipe, CoDeSDPipelineI2I) or isinstance(pipe, SDPipelineI2I)\
        ...
        if (isinstance(scorer, FaceRecognitionScorer) or isinstance(scorer, ClipScorer)):
        ...
        if isinstance(scorer, FaceRecognitionScorer) or isinstance(scorer, ClipScorer):
            # If target image is not used for init_noise conditioning, but only for reward function:
            ...
            # If no target image is present for init_noise conditioning:
