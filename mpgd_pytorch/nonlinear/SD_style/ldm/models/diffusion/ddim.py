...
class DDIMSampler(object):
    ...
    def ddim_sampling(self, cond, shape,
        ...
        for i, step in enumerate(iterator):
            ...
            outs = self.p_sample_ddim_conditional_x0(img, cond, ts, index=index, use_original_steps=ddim_use_original_steps,
    ...
    def p_sample_ddim_conditional_x0(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        if index >= 0.7*self.total_steps:
            repeat = 1
        elif 0.7*self.total_steps > index >= 0.4*self.total_steps:
            repeat = tt
        else:
            repeat = 1 
