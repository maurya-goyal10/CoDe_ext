...
def main(_):
    ...
    #     *[eval_prompt_fn(idx=None) for i in range(config.train.batch_size_per_gpu_available * config.max_vis_images)]
    # )
    # For evaluation only
    eval_prompts, eval_prompt_metadata = zip(
        *[eval_prompt_fn(idx=i) for i in range(config.train.batch_size_per_gpu_available * config.max_vis_images)]
    )    
    # breakpoint()
    if config.resume_from:
