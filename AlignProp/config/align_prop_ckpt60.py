...
def general():
    ...
    config.save_freq = 4
    config.resume_from = "/home/anujinho/PhD_GuidedDiff/AlignProp/logs/unique-disco-47/checkpoints/checkpoint_89"# "/home/anujinho/PhD_GuidedDiff/AlignProp/logs/rural-microwave-8/checkpoints_51-65/checkpoint_60"
...
def evaluate():
    config = general()
    config.reward_fn = "hps"# "aesthetic"
    config.prompt_fn = "eval_hps_v2_all"# "eval_simple_animals"
    config.only_eval = True
    config.same_evaluation = True
    config.max_vis_images = 1000# 300
    config = set_config_batch(config, total_samples_per_epoch=256,total_batch_size= 128, per_gpu_capacity=1)
    return config
